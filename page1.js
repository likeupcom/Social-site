...
const TKBoardStateSchema = new mongoose.Schema({
...
const YTActiveSlotSchema = new mongoose.Schema({
  ...
  TiktokVideo: { type: String, required: true },
...
async function getOrCreateSystemState() {
  let state = await TKBoardState.findOne();
  if (!state) {
    state = new TKBoardState({
...
async function processAppealingPeriodEnd() {
  ...
  try {
    ...
    for (const rejected of rejectedUsers) {
      ...
      await TKWaitingQueue.deleteOne({ _id: rejected._id });
    }

    waitingUsers = await TKWaitingQueue.find().sort({ timestamp: 1 });
...
/**
 * GET /api/Tiktok-dashboard/state
 * Assembles and structuralizes the precise state array display matrix.
 */
router.get("/api/Tiktok-dashboard/state", auth, async (req, res) => {
  try {
    ...
    if (sysState.appealingPeriodActive && sysState.appealingPeriodEnd) {
      ...
      if (remainingMs <= 0) {
        ...
        if (sysState.activeSequenceIndex === 999) {
          ...
          const lockedState = await TKBoardState.findOneAndUpdate(
    ...
    const rawQueue = await TKWaitingQueue.find().sort({ timestamp: 1 });
    ...
    res.json({
      userAccount: { username: dbUser.username, channelUrl: dbUser.tiktok_link || "https://tiktok.com/channel_placeholder" },
      appealingPeriod,
      vipChannels,
      regularChannels,
...
/**
 * POST /api/Tiktok-dashboard/accept-conditions
 */
router.post("/api/Tiktok-dashboard/accept-conditions", auth, async (req, res) => {
  try {
    await connectToDatabase();
    const User = mongoose.model("User");
    const lookupUsername = typeof req.user === "object" ? req.user.username : req.user;
...
/**
 * POST /api/Tiktok-dashboard/verify-visit
 */
router.post("/api/Tiktok-dashboard/verify-visit", auth, async (req, res) => {
  try {
    await connectToDatabase();
...
/**
 * POST /api/Tiktok-dashboard/submit-promotion
 */
router.post("/api/Tiktok-dashboard/submit-promotion", auth, async (req, res) => {
  try {
    await connectToDatabase();
   ...
   const alreadyExists = await YTActiveSlot.findOne({ userId });
    ...
    const cleanVideoUrl = sanitizeTiktokUrl(rawVideoUrl);
    const cleanChannelUrl = sanitizeTiktokUrl(rawChannelUrl);

    // Enforce basic structured regex parameter filters
    if (!cleanVideoUrl.includes("tiktok.com") && !cleanVideoUrl.toLowerCase().includes("tiktok")) {
    ...
    if (dbUser.tiktok_link && sanitizeTiktokUrl(dbUser.tiktok_link) !== cleanChannelUrl) {
    ...
    if (targetPosition === -1) {
      const inQueue = await TKWaitingQueue.findOne({ userId });
      if (!inQueue) {
        const newQueueNode = new TKWaitingQueue({
          userId,
          username: dbUser.username,
          TiktokChannel: cleanChannelUrl,
          TiktokVideo: cleanVideoUrl
...
/**
 * POST /api/Tiktok-dashboard/appeal-user
 */
router.post("/api/Tiktok-dashboard/appeal-user", auth, async (req, res) => {
  try {
    await connectToDatabase();
    ...
    const myAppealsCount = await TKWaitingQueue.countDocuments({
    ...
    const queueRecord = await TKWaitingQueue.findById(queueUserId);
